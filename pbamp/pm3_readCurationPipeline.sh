...
# execute rnammer
parallel -j10 'FOLDER={}; FASTA=${FOLDER%.dir}; cd $FOLDER; rnammer -S bac -multi -gff $FASTA.bac.gff -f $FASTA.bac.fasta $FASTA; cd ..;' ::: roi.$sample.rhq.trim.fwdrev.pol.pick.fasta.*.dir
parallel -j10 'FOLDER={}; FASTA=${FOLDER%.dir}; cd $FOLDER; rnammer -S arc -multi -gff $FASTA.arc.gff -f $FASTA.arc.fasta $FASTA; cd ..;' ::: roi.$sample.rhq.trim.fwdrev.pol.pick.fasta.*.dir
...
parallel -j7 "mafft-qinsi --kimura 1 --thread 5 --quiet {} > 2_mafftQinsi/{/.}.aln" :::: files_to_align.txt
...
# classify all otus
ln -s ../6_rnammer/rnammer.$sample.cons+uncl.16S.fasta
mothur "#classify.seqs(fasta=rnammer.$sample.cons+uncl.16S.fasta, reference=silva.nr_v128.align, taxonomy=silva.nr_v128.tax, processors=30)"
