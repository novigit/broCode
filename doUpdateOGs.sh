...
cat $faas/*.faa | sed -r "/^>/ s/>/>lcl\|/" > $faas/${faas%\/}.faa.db
...
for psiblast in $update/3_psiblast/*.psiblast; do
    ...
    blastdbcmd \
    	-db $faas/${faas%\/}.faa.db \
    	-dbtype prot \
    	-entry_batch <( cut -f 2 $psiblast | sed -r "s/lcl\|//" | sort -u ) \
    	-outfmt %f \
    	>> $update/4_updated_faas/${faabase}_plus_${faas%\/}.faa
done

# align, trim, fastTree, color
mkdir -p $update/5_mafft $update/6_trimal $update/7_fasttree
for updated_faa in $update/4_updated_faas/*.faa; do
    ...
    echo "Inferring fasttree for $faabase ..."
    FastTree -quiet -wag $update/6_trimal/${faabase}_trim.aln \
    	> $update/7_fasttree/${faabase}.tree

    echo "Coloring $faabase.tree ..."
    cd $update/7_fasttree/
