...
# state options
while getopts ":b:g:e:u:t:h" opt; do
    case $opt in
	b) burnin=${OPTARG};;
...
echo "Burnin: $burnin Every: $every Threads: $threads Until: $untils Chains: $chains" > beskow_submit_phylobayes_ppred.debug.out

outname=ppred_b${burnin}_g${generations}_e${every}_u${untils}
...
beskow_submit_phylobayes_ppred.sh -b <burnin> -e <every> -u <until> -t <threads> <chain1> <chain2> <etc>
